...
| DixScript (VS Code) | VS Code extension | ✅ **Open VSX** — [search "DixScript"](https://open-vsx.org/extension/midmanstudio/dixscript) · ⏳ VS Code Marketplace pending |
| VS for Mac · IntelliJ | IDE extensions | ⏳ In progress |
...
```
Full IDE integration (diagnostics, completion, hover). The VS Code extension
below wraps it already — VS for Mac / IntelliJ still need a client wired up.

### VS Code extension
Install **DixScript (.mdix)** by `midmanstudio` from
[Open VSX](https://open-vsx.org/extension/midmanstudio/dixscript) (not yet
on the VS Code Marketplace — Open VSX-compatible editors, e.g. VSCodium,
Cursor, Windsurf, pick it up directly; standard VS Code users can sideload
the `.vsix` from the same page in the meantime).
...
_"Config files shouldn't require a PhD to maintain."_ — The DixScript Philosophy# DixScript: The Swiss Army Knife of Data Formats
