...
<script lang="ts">
  ...
  interface Package {
    ...
    size: number; uploaded: string | null; downloadUrl: string;
    desc: string; tags: string[]; category: string;
    addedBy: string; version: string;
  }
...
{#if modalOpen}
  <RegistryModal filename={modalFile} content={modalContent} loading={modalLoading} error={modalError} on:close={closeModal} />
{/if}
  ...
  <div class="reg-hero">
    <div class="reg-hero-inner">
      ...
      <p class="hero-sub">
        ...
        Click any card to preview its contents.
      </p>
      <div class="hero-usage">
        <span class="usage-label">Quick Start</span>
        <code class="usage-code">Helpers from_cloud "https://dixscript-docs.pages.dev/api/registry/game/game_helpers.mdix"</code>
      </div>
    </div>
  </div>
  ...
  <div class="reg-body">
    ...
    {#if data.error}
      <div class="notice notice--error">
        ...
        <span>Registry unavailable: {data.error}. See the <a href="https://github.com/Mid-D-Man/DixScript-Rust" target="_blank" rel="noopener">GitHub repo</a> for status.</span>
      </div>
    ...
    <div class="reg-footer">
      <div class="reg-footer-inner">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0;color:var(--primary);margin-top:2px">
          <path stroke-linecap="round" stroke-linejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z"/>
        </svg>
        <div>
          <p>
          ...
          <p class="footer-meta-note">
            ...
            this is how package metadata is stored without a database.
          </p>
        </div>
      </div>
    </div>
...
<style>
  ...
  /* Hero */
  .reg-hero { background: var(--card); border-bottom: 1px solid var(--border); padding: 3rem 1.5rem 2.5rem; }
  .reg-hero-inner { max-width: 900px; margin: 0 auto; display: flex; flex-direction: column; gap: 0.875rem; }
  ...
  .hero-sub { font-size: 1rem; color: var(--muted-foreground); line-height: 1.75; max-width: 640px; margin: 0; }
  ...
  .notice--info  { background: rgba(166,124,82,.06); border-color: rgba(166,124,82,.25); color: var(--foreground); }
  .notice--info svg  { color: var(--primary); }
  .notice a { color: var(--primary); text-decoration: underline; }

  /* Grid */
  .pkg-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1.125rem; }

  /* Empty */
  .empty-state { display: flex; flex-direction: column; align-items: center; gap: 0.625rem; padding: 4rem 2rem; text-align: center; }
  .empty-icon {
    width: 3.5rem; height: 3.5rem; background: var(--secondary); border: 1px solid var(--border);
  ...
  .empty-reset {
    margin-top: 0.5rem; background: var(--primary); color: var(--primary-foreground);
    border: none; border-radius: var(--radius); padding: 0.5rem 1.25rem;
