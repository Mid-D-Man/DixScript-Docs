...
<div class="reg-wrap">
  <div class="reg-header">
    <div>
      <h1>Registry</h1>
      <p class="reg-sub">Importable <code>.mdix</code> files — use them in your <code>@IMPORTS</code> section. This is the early version; a full hosted registry with versioning is planned.</p>
    </div>
  </div>

  <div class="reg-notice">
    <strong>How to use:</strong> Download the file, place it next to your <code>.mdix</code> file, then import it: <code>Base from "base_types.mdix"</code>. Access symbols via the alias: <code>Base.Rarity.EPIC</code>, <code>Base.makeItem(...)</code>.
  </div>

  <div class="search-row">
    <input class="search-box" type="text" placeholder="Search packages…" bind:value={search} />
    ...
    <div class="pkg-grid">
      {#each filtered as pkg}
        <div class="pkg-card">
          <div class="pkg-top">
            <div>
              <div class="pkg-name">{pkg.name}<span class="pkg-ver">v{pkg.version}</span></div>
  ...
  <div class="reg-footer-note">
    Want to contribute an importable <code>.mdix</code> file to the registry? Open a PR to the <a href="https://github.com/Mid-D-Man/DixScript-Rust" target="_blank" rel="noopener">DixScript-Rust repo</a> adding your file under <code>mdix_files/shared/</code>.
  </div>
</div>
...
<style>
  ...
  .search-box {
    ...
    color: var(--text); padding: 8px 14px; border-radius: 8px; font-size: 0.875rem;
  ...
  .pkg-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 16px; }
  .pkg-card {
    background: var(--surface); border: 1px solid var(--border);
  ...
  .pkg-name { font-size: 1rem; font-weight: 700; font-family: var(--font-mono); display: flex; align-items: center; gap: 8px; }
  .pkg-ver { font-size: 0.68rem; background: var(--surface2); border: 1px solid var(--border); color: var(--faint); padding: 1px 7px; border-radius: 10px; font-family: inherit; }
  .pkg-desc { font-size: 0.82rem; color: var(--muted); margin-top: 5px; line-height: 1.5; }

  .pkg-tags { display: flex; gap: 6px; flex-wrap: wrap; }
  .tag { font-size: 0.68rem; background: var(--surface2); border: 1px solid var(--border2); color: var(--muted); padding: 2px 8px; border-radius: 10px; }
  ...
  .action-btn {
    ...
    padding: 6px 14px; border-radius: 6px; font-size: 0.80rem;
  ...
  .action-btn.primary { background: rgba(247,129,102,.1); border-color: rgba(247,129,102,.4); color: var(--accent); }
  .action-btn.primary:hover { background: rgba(247,129,102,.2); }

  .empty { text-align: center; padding: 48px; color: var(--faint); }
  .reg-footer-note { margin-top: 32px; padding-top: 20px; border-top: 1px solid var(--border2); font-size: 0.82rem; color: var(--faint); line-height: 1.6; }
</style>
