...
<script lang="ts">
  ...
  const methods = [
    ...
    { name: 'Math.sqrt(x)',          returns: 'double', desc: 'Square root (x must be ≥ 0)' },
    ...
    { name: 'Math.sign(x)',          returns: 'int',    desc: '-1, 0, or 1' },
    ...
    { name: 'Math.exp(x)',           returns: 'double', desc: 'e raised to the power of x' },
